...
Ak program ani neskompiluje
  ...
  - Aj varovania kompilátora môžu poukazovať na chybu v programe.
    - V našich programoch dostávate varovanie, že návratová hodnota funkcie scanf
      sa ignoruje. Táto návratová hondota sa používa na kontrolu správnosti vstupu, ale my zatiaľ 
      budeme predpokladať, že vstup je vždy správny. Toto varovanie teda môžete ignorovať.
...
Ak program dáva zlé výsledky, "cyklí sa" alebo "padá"
  ...
  - To sa dá robiť spúšťaním programu po krokoch v nástroji nazvanom
    debugger (nachádza sa v pokročilejších prostrediach, prípadne existuje
    textový debugger gdb).
...
Na minulej prednáške sme videli príklady, kde spracovávame veľa čísel bez toho, aby sme si ich všetky uložili do pamäte. Počítali sme tak maximum alebo priemer týchto čísel. Teraz si ukážme príklad, kde to takto nevieme.

Chceme spočítať priemer a o každom vstupnom čísle vypísať, či je
nadpriemerné alebo podpriemerné.

  - Priemer vieme až vtedy, keď načítame všetky čísla, musíme si ich teda
    zapamätať
...
int main() {
    const int N = 20; // premennú N už nebude možno meniť, má konštantnú hodnotu 20
    int a[N];
    double sucet = 0;

    printf("Zadavajte %d cisel: ", N);
    for (int i = 0; i < N; i++) {
        scanf("%d", &(a[i]));
        sucet += a[i];
    }
    ...
    for (int i = 0; i < N; i++) {
        if (a[i] > priemer) { 
             printf("%d: vacsie ako priemer.\n", a[i]);
        } else if (a[i] < priemer) {
             printf("%d: mensie ako priemer.\n", a[i]);
        } else {
             printf("%d: priemer.\n", a[i]);
        }
    }
...
Na zamyslenie:
  ...
  - Chyby pri zaokrúhľovaní môžu spôsobiť, že niekedy priemerné
...
```
  ...
  - V starších verziách C to nefunguje, aj keď niektoré novšie
...
```c
// správne: inicializácia zoznamom
int A[4] = {3, 6, 8, 10}; 
// správne: definícia bez inicializácie
int B[4]; 
// nesprávne, pole tu nedefinujeme:
// B = {3, 6, 8, 10};  
// správne - meníme prvky existujúceho poľa:
...
(a==b) { printf("rovnake"); }` síce skompilujete, ale nikdy to nebude
...
int main() {
    ...
    printf("Zadavajte %d cisel z intervalu 0-9: ", N);
    for (int i = 0; i < N; i++) {
        int x;
        scanf("%d", &x);
...
int main() {
  /* Vytvor obrázok so šírkou 150 a výškou 100 a 
