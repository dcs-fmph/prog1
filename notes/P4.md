...
- Od budúceho týždňa budú na niektorých cvičeniach papierové rozcvičky.
- Je to krátky test z učiva, na ktoré už boli príklady na testovači. 
- Bude povolené používať pero a ťahák vo forme jedného obojstranne popísaného listu A4.
- Zakázané sú akékoľvek elektronické zariadenia a komunikácia s inými ľuďmi okrem vyučujúcich.

Prečo robíme takéto rozcvičky a písomné testy na papieri?

- Dobré na precvičenie porozumenia hotovým programom (treba na ďalších
  predmetoch aj pri štúdiu odbornej literatúry).
- Lepšie sústredenie, keď vás nevyrušuje počítač a internet.
...
- Pondelok na cvičeniach papierová rozcvička, vzorové riešenia rozcvičky, možnosť spýtať sa k úlohám, ktoré majú termín do pondelka večera
...
- Prednášky pondelok a stredu ako obvykle
...
- Príklady funkcií z tejto knižnice: `cos(x)`, `sin(x)`, `tan(x)` (tangens), `acos(x)`
  ...
  celá časť), ...
- Pri použití funkcie treba do príkazu na kompilovanie pridať prepínač `-lm`, napríklad:
  `gcc -lm -o program program.c`
- Viac detailov o knižnici možno nájsť [v
  dokumentácii](http://www.cplusplus.com/reference/cmath/).
...
úsečky. Tá môže v C vyzerať napríklad takto:
