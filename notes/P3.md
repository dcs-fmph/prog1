...
- Ak vám testovač k príkladu vypíše zelené OK, prešiel testami. Ak
  vypíše oranžový kód chyby, niečo je zle, treba opraviť a odovzdať znovu.
...
```
  ...
  - Program využíva funkciu `rand()`, ktorá generuje pseudonáhodné celé čísla.
      ...
      - Aby bolo možné použiť túto funkciu, treba do hlavičky pridať
        ` #include  `<stdlib.h>.
      - Výstupom funkcie `rand()` je celé číslo medzi nulou a
...
Pri písaní programov myslite na to, že ich často budú okrem počítača
...
čitateľnosť zdrojového kódu zlepšujú:
  ...
  - **Odsadzovanie:** príkazy vykonávané v cykle, či v podmienke (alebo
    ...
    niekoľko pozícií doprava (napríklad o 4 medzery).
      ...
      - Väčšina editorov pre C nejakým spôsobom odsadzovanie
        podporuje
...
Túto úlohu budeme riešiť programom pracujúcim nasledovne:
  ...
  - V premennej `ucet` budeme uchovávať aktuálny stav účtu
...
Všimnite si, že pri načítaní úroku sme použili `%%`, aby sa vypísal znak `%`. V opačnom prípade by príkaz `printf` očakával za znakom `%` typ premennej, ktorý sa má vypísať, napríklad `%d` pre `int`, `%lf` pre `double`, atď.
...
V C existuje dvojica príkazov umožňujúcich umelo prerušiť
